...
const MediaItem: React.FC<MediaItemProps> = ({
  ...
  if (error) {
    return (
      <div className="rounded-lg bg-red-100 p-4 text-sm text-red-500 dark:bg-red-900 dark:text-red-300">
  ...
  if (!visible && !isExpanded) {
    return <div ref={ref} className="h-64 animate-pulse rounded-lg bg-gray-100 dark:bg-gray-800" />;
  }
      ...
      <img
        ...
        className={
          isExpanded
            ...
            : 'h-full max-h-64 w-full cursor-pointer rounded-lg bg-black object-contain hover:opacity-90'
        }
        onClick={onClick}
        onLoad={onLoad}
        onError={() => {
      ...
      {isExpanded && getMediaType(validUrls[expandedIdx!]) === 'image' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75">
          <div className="relative h-screen w-screen p-8">
            <button
              className="absolute top-8 right-8 size-10 cursor-pointer rounded-full bg-black/50 p-2 text-white hover:bg-black/75"
